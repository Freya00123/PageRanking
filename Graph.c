...
static AdjList newAdjNode(Vertex v);
static AdjList adjListInsert(AdjList l, Vertex v);
...
void GraphInsertEdge(Graph g, Vertex src, Vertex dest) {
	assert(g != NULL);
	assert(validVertex(g, src));
	assert(validVertex(g, dest));

	g->outLinks[src] = adjListInsert(g->outLinks[src], dest);
	g->inLinks[dest] = adjListInsert(g->inLinks[dest], src);
}
